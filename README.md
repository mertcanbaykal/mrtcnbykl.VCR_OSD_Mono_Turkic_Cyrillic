## VCR_OSD_Mono_+\_Turkic_+\_Cyrillic\_(mrmanet_mrtcnbykl_Daymarius)

<p align="center">
    <a href="README.md">Türkçe</a>
    ·
    <a href="README_EN.md">English</a>
</p>

### İNDİRME BAĞLANTISI

- [VCR_OSD_Mono_+\_Turkic_+\_Cyrillic\_(mrmanet_mrtcnbykl_Daymarius).ttf](https://drive.google.com/file/d/1hNxnTeYSIN-2HnTKR9--5zaEaPU9_bqN/view?usp=sharing) bağlantısı ile yazı tipini indirebilirsiniz.

### Yazı Tipi İle İlgili

*mrmanet (Riciery Leal)*'in simgeler ve Avrupa Latin harflerini içeren tasarımları ile *Daymarius*'un Kiril harflerini içeren tasarımlarını birleştirdim. Derleme çalışmanın üzerine ek olarak Ortak Türk Abecesindeki tüm harfleri ekledim. Unity Oyun Motoru'nda kullanırken düz yazı içerisinde düzensizlik oluşturan harfleri ortalayarak biçimlendirdim.

Ortak Türk Abecesi: [https://tr.wikipedia.org/wiki/Ortak_Türk_alfabesi](https://tr.wikipedia.org/wiki/Ortak_Türk_alfabesi)

### Kullanım İzni

Benim eklediğim Türk harflerini herkes düşlediği biçimde kullanabilir, ancak; özgün yazı tipinin tasarımcısı *mrmanet* ve Kiril harflerinin tasarımcısı *Daymarius*'un kullanım izni ile ilgili görüşlerini de inceleyiniz.

Onların çalışmaları:
- [VCR OSD Mono](https://www.dafont.com/vcr-osd-mono.font)
- [VCR OSD Mono [RUS by Daymarius]](https://online-fonts.com/fonts/vcr-osd-mono-rus-daymarius)



## *mrtcnbykl*
*[mrtcnbykl](https://github.com/mertcanbaykal/mrtcnbykl) bağlantısı ile öteki çalışmalarıma ulaşabilirsiniz.*
